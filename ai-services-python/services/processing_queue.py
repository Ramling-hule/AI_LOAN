...
class ProcessingQueue:
    ...
    async def _worker_loop(self):
        while self._running:
            try:
                ...
                await execute("UPDATE loan_processing_jobs SET status = 'running', updated_at = CURRENT_TIMESTAMP WHERE id = $1", self.current_job_id)
                ...
                try:
                    ...
                    await execute("UPDATE loan_processing_jobs SET status = 'paused', updated_at = CURRENT_TIMESTAMP WHERE id = $1", self.current_job_id)
                finally:
    ...
    async def _process_job(self, job: dict):
        ...
        try:
            if task_type == 'extraction':
                ...
                await execute(
                    "UPDATE loans SET ai_extraction_id = $1, ai_extraction_status = 'completed', extracted_summary = $3::jsonb WHERE id = $2",
                ...
                await execute(
                    "UPDATE loans SET underwriting_assessment = $1::jsonb, risk_score = $2 WHERE id = $3",
                ...
                await execute(
                    "UPDATE loans SET ai_extraction_id = $1, ai_extraction_status = 'completed', extracted_summary = ($3::text)::jsonb WHERE id = $2",
                    ext['extraction_id'], job['loan_id'], json.dumps(summary)
                )
                logger.info(f"[Queue] Extraction complete for job {job_id}. Sleeping 20s to respect API quotas.")
                ...
                await execute(
                    "UPDATE loans SET underwriting_assessment = ($1::text)::jsonb, risk_score = $2 WHERE id = $3",
                    json.dumps(assessment), assessment.get('risk_score', 0), job['loan_id']
                )
            else:
                raise ValueError(f"Unknown task type: {task_type}")

            await execute("UPDATE loan_processing_jobs SET status = 'completed', updated_at = CURRENT_TIMESTAMP WHERE id = $1", job_id)
            ...
            await execute("UPDATE loan_processing_jobs SET status = 'failed', error_message = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2", str(e), job_id)
    ...
    async def preempt(self, admin_loan_id: str, task_type: str, payload: dict) -> str:
        ...
        if self.current_task and not self.current_task.done():
            ...
            current_job = await fetchrow("SELECT loan_id FROM loan_processing_jobs WHERE id = $1", self.current_job_id)
            if current_job and current_job['loan_id'] != admin_loan_id:
                logger.warning(f"[Queue] Preempting current job {self.current_job_id} for Admin Loan {admin_loan_id}")
                self.current_task.cancel()
